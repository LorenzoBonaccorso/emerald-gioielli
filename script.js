...
const PUREZZA_ARGENTO_925 = 0.925;
const SOGLIA_PROMO_GRAMMI = 30;

// Regole commerciali: percentuali applicate ai valori di mercato.
const PERCENTUALE_ACQUISTO_STANDARD_18K = 0.845;
const PERCENTUALE_ACQUISTO_PROMO_18K = 0.885;
const PERCENTUALE_ACQUISTO_ARGENTO_925 = 0.70;

const ARROTONDAMENTO_PREZZI_18K = 0.5;
const ARROTONDAMENTO_ARGENTO_925 = 0.01;
const INTERVALLO_AGGIORNAMENTO_MS = 60000;
const CACHE_KEY_PREZZI = 'emerald-metals-live-prices-v2';
...
const priceSilver925Element = document.getElementById('price-silver925');
...
let prezzoArgento925EurGrammo = null;
...
function formatEuroPrezzo18(value) {
 ...
 return formatEuro(value, isInteger ? 0 : 1, 1);
}
...
function getPrezzoAssetPerGrammi(asset, grammi) {
 if (asset === 'silver925') return prezzoArgento925EurGrammo;
...
function calcolaPrezziDaMercato(prezzo24EurGrammo, prezzoArgentoPuroEurGrammo) {
 ...
 const argento925 = arrotondaAlPasso(
 prezzoArgentoPuroEurGrammo * PUREZZA_ARGENTO_925 * PERCENTUALE_ACQUISTO_ARGENTO_925,
 ARROTONDAMENTO_ARGENTO_925
 );
 return { standard18, promo18: Math.max(promo18, standard18), argento925 };
}

function salvaPrezziInCache() {
 if (![prezzoOroPuroEurGrammo, prezzoStandard18EurGrammo, prezzoPromo18EurGrammo, prezzoArgento925EurGrammo].every(Number.isFinite)) return;
 try {
 ...
 argento925: prezzoArgento925EurGrammo
 }));
 } catch (error) {
...
function caricaPrezziDaCache() {
 ...
 const valuesValid = cached && [cached.prezzo24, cached.standard18, cached.promo18, cached.argento925].every(Number.isFinite);
 ...
 prezzoArgento925EurGrammo = cached.argento925;
...
function aggiornaSchedePrezzi() {
 ...
 if (priceSilver925Element) priceSilver925Element.textContent = formatEuro(prezzoArgento925EurGrammo, 2, 2);
 if (gold18OfferTitle) {
...
function getAssetLabel(asset) {
 ...
 if (asset === 'silver925') return 'Argento 925';
 return 'Oro 18K';
}
...
function aggiornaConversione() {
 ...
 converterNoteElement.textContent = `Prezzo di acquisto dell'argento 925: ${formatEuro(prezzoArgento925EurGrammo, 2, 2)} €/g.`;
 }
}
...
function aggiornaSelezioneInterfaccia() {
 ...
 if (goldPanel) goldPanel.classList.toggle('active', currentAsset !== 'silver925');
 if (silverPanel) silverPanel.classList.toggle('active', currentAsset === 'silver925');
 ...
 if (calculatorPanel) {
  const isSilverTheme = currentAsset === 'silver925';
...
function setCurrentAsset(asset) {
 const nextAsset = asset === 'silver925' ? 'silver925' : 'gold18';
...
async function aggiornaPrezziLive() {
 ...
 prezzoArgento925EurGrammo = prezzi.argento925;
 ultimoAggiornamentoPrezzo = new Date();
 usingCachedPrices = false;
 salvaPrezziInCache();
 } catch (error) {
 console.warn('Aggiornamento prezzi non disponibile:', error);
 if (![prezzoOroPuroEurGrammo, prezzoPromo18EurGrammo, prezzoArgento925EurGrammo].every(Number.isFinite)) {
 caricaPrezziDaCache();
 } else {
 usingCachedPrices = true;
 }
...
if (calculatorToggle && calculatorPanel) {
 ...
 calculatorToggle.setAttribute('aria-expanded', String(willOpen));
 if (calculatorToggleLabel) calculatorToggleLabel.textContent = willOpen ? 'Nascondi il calcolo' : 'Calcola il valore';
...
function scrollToSectionView(targetId, highlightAfterScroll = true) {
 ...
 /* Tornando alla sezione quotazioni, mostra prima il quadro sintetico completo.
    Il calcolatore resta disponibile con il suo pulsante, ma non allunga il blocco durante la navigazione. */
 if (targetId === '#quotazione' && calculatorToggle && calculatorPanel) {
  calculatorPanel.classList.add('is-collapsed');
  calculatorPanel.setAttribute('aria-hidden', 'true');
  calculatorToggle.setAttribute('aria-expanded', 'false');
  if (calculatorToggleLabel) calculatorToggleLabel.textContent = 'Calcola il valore';
 }
