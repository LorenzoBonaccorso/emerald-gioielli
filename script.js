...
const RIDUZIONE_ACQUISTO_ARGENTO = 0.32;

const ARROTONDAMENTO_PREZZI_18K = 0.1;
const ARROTONDAMENTO_ARGENTO_800 = 0.01;
const INTERVALLO_AGGIORNAMENTO_MS = 10 * 60 * 1000;
const CACHE_KEY_PREZZI = 'emerald-metals-live-prices-v6';
...
async function caricaMercatoDaTickBaseStatico() {
 // Il file viene rigenerato automaticamente da GitHub Actions ogni 10 minuti.
 // La chiave TickBase resta nei Secrets del repository e non arriva mai al browser.
 const cacheBuster = Math.floor(Date.now() / INTERVALLO_AGGIORNAMENTO_MS);
 const data = await fetchJsonWithTimeout(`data/market-prices.json?v=${cacheBuster}`, 12000);
 const oroEurOncia = Number(data && data.xau_eur_per_ounce);
 const argentoEurOncia = Number(data && data.xag_eur_per_ounce);
 if (!data || data.success !== true || ![oroEurOncia, argentoEurOncia].every(value => Number.isFinite(value) && value > 0)) {
  throw new Error((data && data.message) || 'Quotazione TickBase non ancora disponibile');
 }
 return {
  oroEurOncia,
  argentoEurOncia,
  asOf: data.as_of ? new Date(data.as_of) : new Date(),
  stale: Boolean(data.stale),
  provider: data.provider || 'TickBase',
  mode: data.price_mode || 'bid'
 };
}
...
async function caricaDatiMercato() {
 try {
  ...
  console.warn('File TickBase non disponibile o non aggiornato; uso fonti di riserva:', error);
